...
num_partitions = 3      # partition 
redis_conn = redis.Redis(host='redis', port=6379, db=0)


dag = DAG(
    dag_id = "kpop_spotify_data_extract_add",
...
def start():
    ...
    spotify_util.set_access_tokens(client_ids, client_secrets)
...
def _search_kpop_artist():
    ...
    artist_id_list = [] # redis에 넘겨줄 artist_id
    ...
    while True: 
        ...
        if response.status_code == 200 :
            ...
            for idx, artist in enumerate(artists[:5]) :
                ...
                log.info(f"ARTIST [{offset + (idx+1)}/{total_artist}] || artist_id :: {artist_id} artist_name :: {artist_name} S3 Bucket :: raw_data/spotify/api/artist/{ymd}/{artist_id}.json")
                cnt += 1
    ...
    log.info(f"{json.dumps(artist_id_list)}")
...
def _scarping_kpop_album():
    artist_id_list = json.load
